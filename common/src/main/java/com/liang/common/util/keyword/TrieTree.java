		...
		for (int i = 0; i < source.length; i++) {
			...
			while (i < source.length && k < target.length) {
				...
				if (source[i] == target[k]) {
					...
					//liang添加了以下内容
					//如果已经检测了和要替换的文字长度一样的字数，则本轮跳过
					lengthCount++;

					if(lengthCount==target.length){
						//如果长度和替换的文字相同，则i要往回退一下
						//比如“吉度未笋吾送点危乔限。李白李白”这句话，要查找李白这个词。
						//当i=11的时候第一次匹配，此时k=0,然后k和i都加1，此时i=12，k=1，可以匹配，此时已经匹配了两个字，
						//则不需要再往后匹配了，但是此时回到for循环，i又被加1，变成14，相当于跳过了一个13，所以此时i得减1
						i--;
						break;
					}
				}
